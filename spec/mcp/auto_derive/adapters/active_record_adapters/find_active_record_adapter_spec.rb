...
require 'spec_helper'
require 'mcp/auto_derive/adapters/active_record_adapters/find_active_record_adapter'
...
RSpec.describe FastMcp::AutoDerive::Adapters::ActiveRecordAdapters::FindActiveRecordAdapter do
  describe '.create_tool' do
    ...
    it 'creates a subclass with correct parameters' do
      ...
      expect(described_class).to have_received(:create_subclass).with(
        hash_including(
          name: 'find_active_record_model',
          class_name: 'ActiveRecord',
          method_name: :find,
          description: 'Find a record by model name and ID'
        )
      )
    end
  ...
  describe '.configure_as_fast_mcp_tool' do
    ...
    it 'sets constants for the tool in appropriate namespaces' do
      ...
      expect(FastMcp::AutoDerive::Tools.const_get('FindActiveRecordModel')).to eq(subclass)
      expect(Object.const_get('MCPFindActiveRecordModel')).to eq(subclass)
      expect(Object.const_get('ToolsFindActiveRecordModel')).to eq(subclass)
    end
  end
  ...
  describe '.define_call_method' do
    ...
    before do
      ...
      allow(record).to receive(:as_json).and_return({ id: '123', name: 'Test Record' })
    ...
    it 'implements a call method that finds a record' do
      instance = klass.new

      allow(instance).to receive(:serialize_result).and_return({ id: '123', name: 'Test Record' })
      ...
      expect(result).to eq({ id: '123', name: 'Test Record' })
    end
  ...
  describe '.subclass_params' do
    it 'returns the correct parameters for creating a subclass' do
      ...
      expect(params[:name]).to eq('find_active_record_model')
      expect(params[:class_name]).to eq('ActiveRecord')
      expect(params[:method_name]).to eq(:find)
      expect(params[:description]).to eq('Find a record by model name and ID')
